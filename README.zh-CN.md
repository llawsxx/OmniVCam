...
`<DSHOW>` 是原生 DirectShow 采集路径。它不同于使用 `format=dshow` 的 FFmpeg 输入：OmniVCam 会在自身内部创建 DirectShow 图，并将采集帧送入常规的视频/音频滤镜和输出管线。压缩样本会复制到有界实时队列，再由独立工作线程解码，避免编解码工作阻塞 DirectShow sample callback。
